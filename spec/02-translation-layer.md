...
- **Transport descriptor fields**: capability `transport` (with sub-capabilities `move_to_pose` / `follow_trajectory` / `handoff` / `carry` / `lift` / `lower`), `embodiment.limits.a_cartesian_max`. Swept-volume collision query (above) must include the **held object**, not only the effector.
...
- **Compliance capability + force descriptor fields**: capability `force` (with sub-capabilities insert_fit / push / pull / …) and a declared **compliance** capability (`passive` / `active` / `virtual force control`). `force` primitives request a compliance mode in parameters and assert it as a precondition; a rigid-only embodiment cannot run them.
