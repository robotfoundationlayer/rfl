...
//! Conformance test class 2 for the remaining reach primitives (reach.to_pose / reach.approach):
//! the retarget output is byte-deterministic, matches a committed golden, and every line is a valid
//! driver-interface execute message. Both are free-space terminal-postcondition moves and reach.*
//! is the unkeyed baseline (no capability gate), so the skill retargets to every hand unchanged.
...
#[test]
fn golden_reach_allegro() {
    insta::assert_snapshot!("reach_allegro", jsonl_for("allegro"));
}

#[test]
fn golden_reach_leap() {
    insta::assert_snapshot!("reach_leap", jsonl_for("leap"));
}

#[test]
fn golden_reach_pneumatic() {
    insta::assert_snapshot!("reach_pneumatic", jsonl_for("pneumatic-6f"));
}

#[test]
fn reach_generation_is_byte_identical() {
...
#[test]
fn reach_every_line_is_a_valid_execute_message() {
    let schema_path =
