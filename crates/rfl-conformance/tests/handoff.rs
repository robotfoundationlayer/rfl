...
//! Conformance test class 2 for transport.handoff: the retarget output is byte-deterministic,
//! matches a committed golden, and every line is a valid driver-interface execute message. (The
//! GC6 two-party verification is in battery.rs.)
...
#[test]
fn golden_handoff_allegro() {
    insta::assert_snapshot!("handoff_allegro", jsonl_for("allegro"));
}

#[test]
fn golden_handoff_leap() {
    insta::assert_snapshot!("handoff_leap", jsonl_for("leap"));
}

#[test]
fn golden_handoff_pneumatic() {
    insta::assert_snapshot!("handoff_pneumatic", jsonl_for("pneumatic-6f"));
}

#[test]
fn handoff_generation_is_byte_identical() {
...
#[test]
fn handoff_every_line_is_a_valid_execute_message() {
    let schema_path =
