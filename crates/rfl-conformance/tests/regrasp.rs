...
//! Conformance test class 2 for in_hand.regrasp: the retarget output is byte-deterministic,
//! matches a committed golden, and every line is a valid driver-interface execute message. (The
//! GC3 make-before-break verification is in battery.rs / envelope_conformance.rs.)
...
#[test]
fn golden_regrasp_allegro() {
    insta::assert_snapshot!("regrasp_allegro", jsonl_for("allegro"));
}

#[test]
fn golden_regrasp_leap() {
    insta::assert_snapshot!("regrasp_leap", jsonl_for("leap"));
}

#[test]
fn golden_regrasp_pneumatic() {
    insta::assert_snapshot!("regrasp_pneumatic", jsonl_for("pneumatic-6f"));
}

#[test]
fn regrasp_generation_is_byte_identical() {
...
#[test]
fn regrasp_every_line_is_a_valid_execute_message() {
    let schema_path =
