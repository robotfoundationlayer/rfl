...
//! Conformance test class 2 for the surface-scan arc variant: pattern: arc
//! compiles an Arc region into a swept-arc Σ at radius = standoff about the
//! pivot, bore pointing inward (spec/02 Appendix A, arc). Byte-deterministic,
...
#[test]
fn golden_arc_allegro() {
    insta::assert_snapshot!("arc_allegro", jsonl_for("allegro"));
}

#[test]
fn golden_arc_leap() {
    insta::assert_snapshot!("arc_leap", jsonl_for("leap"));
}

#[test]
fn golden_arc_pneumatic() {
    insta::assert_snapshot!("arc_pneumatic", jsonl_for("pneumatic-6f"));
}

#[test]
fn arc_generation_is_byte_identical() {
...
#[test]
fn arc_every_line_is_a_valid_execute_message() {
    let schema_path =
