...
- **If you want the strategic / institutional / network-effect framing as well**: read the position paper (`RFL_v1.0_en.pdf`), the **G-1 Hybrid stewardship edition** (RFL Inc. + a Lead Customer Program, with a staged path to a neutral Foundation, on the Confluent / Apache Kafka precedent). It overlaps with the spec extract on the technical core (§§ 3–4) and adds: the Master Equation derivation (an explicitly illustrative model, § 5), the nine reinforcement loops (§ 5), the G-1 Hybrid stewardship design — RFL Inc. and the path to a neutral Foundation (§ 7), the Crémer-McLean structural analogy and empirical-signals discussion for bilateral compounding, the Call to Action and Lead Customer Program (§ 9), and Comparative Analysis at greater length (§ 8). The position paper's own preface marks it as a position paper (not a peer-reviewable academic paper) and explains the choice.
